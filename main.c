...
int main(int argc, char *argv[]) {
  ...
  Ninfected_ptr = malloc( 5 + ( (int) (90*K*K*(1 + log(G->V) )  ) ) * sizeof *Ninfected_ptr );
  ...
  stable_infect(K, (unsigned int) U, objtype);
  int i ;
  for(i = 1; i <= G->V ; ++i){
    if(G->adj_list[i]->prob > 0){
      fprintf(stdout, "\n %d: prob %f\n", G->adj_list[i]->label, G->adj_list[i]->prob/ (float)MAX_EDGE_WEIGHT) ;
    }
  }
  fclose(out_graph);
