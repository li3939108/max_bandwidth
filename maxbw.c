...
int *maxbw_dkt_no_heap(Graph *G, int s_label, int t_label)
{
    ...
    while (!(hash[t_label] && hash[t_label]->in_tree))
    {
        ...
        for (i = 0; i < u->degree; i++)
        {
            ...
            int new_bandwidth =
                    parent_bandwidth > v_label_weight[1] ? v_label_weight[1]
                                                         : parent_bandwidth;
            if (hash[v_label_weight[0]] &&
                new_bandwidth > hash[v_label_weight[0]]->value)
            {
 ...
 */
    ...
    int *parent = (int *) malloc(
            (G->V + 1) * sizeof *parent), i, j, (*e)[2], (*ep)[2];
    edges(G, NULL);
    e = G->edge_list;
    ep = G->edge_pair;
    ...
    while (h->size > 1 &&
           find(G->adj_list[s_label]) != find(G->adj_list[t_label]))
    {
        int key = h->keys[1];
        int value = pop(h, 0);
