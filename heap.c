...
Heap *new_heap(int max_size, int size, int (*key_value_pairs)[2], heap_t min_or_max){
	...
	if(key_value_pairs == NULL){
		...
		for(i = 0; i < max_size + 1; i++){
			h->indices[i] = i ;
			h->keys[i] = i ;
			h->values[i] = rand() % VALUE_RANGE ;
		}
	}else{
		h->indices = (int *)calloc( (max_size + 1) , sizeof *(h->indices)) ;
		h->keys = (int *)calloc( (max_size + 1) , sizeof *(h->keys)) ;
		h->values = (int *)calloc( (max_size + 1) , sizeof *(h->values)) ;
		h->size = size ;
		for(i = 0; i < size ; i++){
			h->keys[i + 1] = key_value_pairs[i][0] ;
			h->values[ key_value_pairs[i][0] ] = key_value_pairs[i][1] ;
			h->indices[ key_value_pairs[i][0] ] = i + 1 ;
		}
	}
